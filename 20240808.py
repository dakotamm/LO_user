...
fig, ax = plt.subplot_mosaic(mosaic, figsize=(16,9), layout='constrained')


ax_name = 'map'

ax[ax_name].pcolormesh(plon, plat, zm_inverse, linewidth=0.5, vmin=-100, vmax=0, cmap = 'gray')
...
for n in range(len(site_labels)):
    ...
    ax[ax_name].text(path.vertices[0][0], path.vertices[0][1] +0.02, str(num) + '. ' + site.replace('_', ' ').title(), color= 'blue')
...
for var in ['deep_DO_mg_L', 'surf_CT', 'strat_sigma']:
    ...
    if var == 'deep_DO_mg_L':
        ...
        ylabel = r'$\delta$ Bottom 20% - Surface 5m Density [$\sigma$/century]'
    ...
    for stat in ['mk_ts']:
        ...
        for deep_DO_q in ['deep_DO_q50']:
            ...
            sns.scatterplot(data = plot_df, x= 'site_num', y = 'slope_datetime_cent_95hi', color = 'b', alpha =0.7, ax = ax[ax_name], s= 10, legend=False)
    
            sns.scatterplot(data = plot_df, x= 'site_num', y = 'slope_datetime_cent_95lo', ax = ax[ax_name], s= 10, legend=False)
    
            sns.scatterplot(data = plot_df, x= 'site_num', y = 'slope_datetime_cent', alpha =0.7, ax = ax[ax_name])
            ...
            ax[ax_name].grid(color = 'lightgray', linestyle = '--', alpha=0.2)
                                    
            ax[ax_name].axhline(0, color='gray')
            
            ax[ax_name].set_ylabel(ylabel)
...
plt.savefig('/Users/dakotamascarenas/Desktop/pltz/for_Parker.png', bbox_inches='tight', dpi=500,transparent=True)
