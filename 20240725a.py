...
for var in ['DO_mg_L']:
    ...
    for site in ['point_jefferson']:
        ...
        fig, ax = plt.subplots(ncols=6, figsize = (16,4), sharey=True, sharex=True)
        ...
        for summer in summers_list:
            ...
            for season in ['summer']:
                ...
                sns.scatterplot(data=plot_df, x='datetime', y ='val', ax=ax[c], hue='surf_deep', palette=colors, alpha=0.7, legend=tf)
                ...
                for depth in ['surf', 'deep']:
                    ...
                    plot_stats = all_stats[(all_stats['site'] == site) & (all_stats['stat'] == 'mk_ts') & (all_stats['time_avg'] == 'none') & (all_stats['summer_def'] == summer) & (all_stats['var'] == var_name)]
                    
                    pdf = plot_df[plot_df['surf_deep'] == depth]
                    ...
                    p = plot_stats['p'].unique()
                    
                    slope_datetime_unc_cent = plot_stats['slope_datetime_unc_cent'].unique()
                    
                    B1 = plot_stats['B1'].unique()
                    
                    B0 = plot_stats['B0'].unique()
                    
                    if p < alpha:
                        
                        ax[c].plot([x_plot.min(), x_plot.max()], [B0 + B1*x.min(), B0 + B1*x.max()], color=color, alpha =0.7)
                        
                    #     ax[c].text(0.98,y, slope_datetime_unc_cent, horizontalalignment='right', verticalalignment='top', transform=ax[c].transAxes, color=color, bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))
            ...
            ax[c].set_xlim([datetime.date(1930,1,1), datetime.date(2024,12,31)])
            
            ax[c].tick_params(rotation=90) #, ha='right')

            
            ax[c].set_xlabel('')
            
            ax[c].set_title(summer)
        ...
        plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_' +var + '_allsummers_mkts.png', bbox_inches='tight', dpi=500, transparent=True)
# %%
