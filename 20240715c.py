...
for site in odf_use['site'].unique():
    ...
    fig, ax = plt.subplot_mosaic(mosaic, layout='constrained', figsize = (8,12), sharex=True)
    
    ax_name = 'skagit'
    
    plot_df = annual_skagit_df
    
    sns.lineplot(data=plot_df, x='datetime', y ='mean_va', color = 'black', ax=ax[ax_name], alpha=0.7, legend = False)
    
    ax[ax_name].grid(color = 'lightgray', linestyle = '--', alpha=0.5)
    
    ax[ax_name].set_ylabel('Mean Discharge [cfs]')
    
    ax[ax_name].set_ylim(0,23000)
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_surf_deep_decadal_wskagit.png', bbox_inches='tight', dpi=500, transparent=False)