...
Usage:
    python wind_flow_stats.py -gtx wb1_r0_xn11b -mooring M1 -year 2017
"""
...
def main():
    ...
    for x_base, y_base in HOURLY_PAIRS:
        ...
        if xc not in df.columns or yc not in df.columns:
            ...
            continue
        # Full record
        rs_full = corr_curve(df, xc, yc, HOURLY_LAGS_H)
        lag_f, r_f = best_lag(HOURLY_LAGS_H, rs_full)
        # Event-window subset
        sub = df.loc[mask_event]
        rs_evt = corr_curve(sub, xc, yc, HOURLY_LAGS_H)
        lag_e, r_e = best_lag(HOURLY_LAGS_H, rs_evt)
        rows.append({
            'x': x_base, 'y': y_base, 'window': 'full_year',
            'best_lag_h': lag_f, 'best_r': r_f, 'n': int(np.isfinite(df[xc]).sum()),
        })
        rows.append({
            'x': x_base, 'y': y_base, 'window': 'leadup_concat',
            'best_lag_h': lag_e, 'best_r': r_e, 'n': int(np.isfinite(sub[xc]).sum()),
        })
        curves[f'{x_base}->{y_base} full']   = rs_full
        curves[f'{x_base}->{y_base} leadup'] = rs_evt
    ...
    # Use best lag from full-record tau_along -> bot_DO
    best = next((r for r in rows
                 if r['x'] == 'tau_along' and r['y'] == 'bot_DO_mgL'
                 and r['window'] == 'full_year'), None)
    reg_rows = []
    if best is not None and np.isfinite(best['best_lag_h']):
        ...
        for label, subset in [('full_year', df2),
            ...
            res = ols_regression(subset, ['tau_along_lag', 'tau_across_lag'],
                                 'bot_DO_mgL' + suf)
            res.update({'window': label, 'lag_h': L,
                        'predictors': 'tau_along_lag+tau_across_lag',
                        'target': 'bot_DO_mgL' + suf})
            reg_rows.append(res)
            print(f'  OLS [{label:14s}] lag={L}h '
                  f'R2={res["R2"]:+.3f} '
    ...
    if curves:
        ...
        im = ax.pcolormesh(HOURLY_LAGS_H, np.arange(len(labels) + 1),
