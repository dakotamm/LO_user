...
while dt <= dt1:
    ...
    while blow_ups <= blow_ups_max:
        ...
        if args.run_roms:
            ...
            # these are for checking on the run using squeue - THIS WHOLE SECTION MODIFED by DM from AL on 20250716
            if 'klone' in Ldir['lo_env']:
                # cmd_list = ['squeue', '-p', args.cpu_choice, '-A', args.group_choice] # DM - this is the default line edited in this section on 20250716
                cmd_list = ['squeue', '-p', 'ckpt-g2', '-u', 'dakotamm'] # recommendation from Jilian
