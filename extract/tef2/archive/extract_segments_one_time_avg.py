"""
This code is the main worker for extract_segments_avg.py.

Adapted from extract_segments_one_time.py to work with avg files.
Key difference: computes EminusP from evaporation and rain fields
since avg files don't have a pre-computed EminusP variable.

EminusP = (evaporation - rain) / rhow

where evaporation and rain are in kg/m2/s and rhow = 1000 kg/m3 (freshwater density,
as used in ROMS mod_scalars.F), giving EminusP in m/s.
"""
...
parser.add_argument('-in_fn', type=str) # path to an average file
...
# EminusP: compute from evaporation and rain
# evaporation [kg/m2/s], rain [kg/m2/s], rhow = 1000 kg/m3
# EminusP [m/s] = (evaporation - rain) / rhow
rhow = 1000.0  # freshwater density [kg/m3], same as ROMS mod_scalars.F
if 'evaporation' in ds.data_vars and 'rain' in ds.data_vars:
    evap = ds.evaporation[0,:,:].values
    rain = ds.rain[0,:,:].values
    two_d['EminusP'] = (evap - rain) / rhow
elif 'EminusP' in ds.data_vars:
