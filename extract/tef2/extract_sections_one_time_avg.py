"""
Function to do the extraction of all sections for a single average file,
using Huon and Hvom (volume flux) instead of velocity.
"""
...
huu = Hu[:, u_df.j, u_df.i] * u_df.pm.to_numpy().reshape(1,-1)
hvv = Hv[:, v_df.j, v_df.i] * v_df.pm.to_numpy().reshape(1,-1)
# merge u and v parts back into one
q = nan * ones(CC['salt'].shape)
q[:,u_df.index] = huu
q[:,v_df.index] = hvv
CC['q'] = q
