"""
Code to extract tef2 sections from average files, using Huon/Hvom volume flux.

To test on mac:
run extract_sections_avg.py -gtx cas7_trapsV00_meV00 -ctag c0 -get_bio True -0 2017.07.04 -1 2017.07.06
...
temp_dir = out_dir0 / ('temp_avg_' + Ldir['ds0'] + '_' + Ldir['ds1'])
...
for ii in range(N):
    ...
    # use subprocesses - call the avg worker that uses Huon/Hvom
    cmd_list = ['python3', 'extract_sections_one_time_avg.py',
...
Data variables:
    ...
    q        (time, z, p) float64 ...   <-- volume flux from Huon/Hvom
