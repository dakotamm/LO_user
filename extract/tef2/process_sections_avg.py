"""
Process tef2 extractions from average files (Huon/Hvom), giving transport vs. salinity for:
volume, salt, and other variables.

PERFORMANCE: 21 seconds for test.

To test on mac:
run process_sections_avg.py -gtx cas7_trapsV00_meV00 -ctag c0 -0 2017.07.04 -1 2017.07.06
...
out_dir = out_dir0 / ('processed_avg_' + Ldir['ds0'] + '_' + Ldir['ds1'])
Lfun.make_dir(out_dir, clean=True)

sect_list = [item.name for item in in_dir.glob('*.nc')]
if Ldir['testing']:
    # just the first section of whatever collection this is. Was hardwired to
    # 'pc0.nc', which is not in every collection -- e.g. wb1_pc1 has no pc0.
    sect_list = sorted(sect_list)[:1]
...
print('\nProcessing TEF extraction (avg/Huon):')
print(str(in_dir))
...
for ext_fn in sect_list:
    ...
    # Use q directly from Huon/Hvom extraction (already volume flux in m3/s)
    q = ds['q'].to_numpy()
    V['q'] = q
    ot = ds['time'].to_numpy()
    ...
    for tt in range(NT):
        ...
        for vn in QV.keys():
            ...
            if vn == 'q':
                ...
                qnet[tt] = np.nansum(XF)
                # and tidal energy flux
                zi = zeta[tt,:]
                zi[np.isnan(QV[vn][tt,0,:])] = np.nan
                ssh[tt] = np.nanmean(zi)
