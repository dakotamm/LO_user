...
if testing:
    vol_list = ['Penn Cove'] #['Puget Sound']
else:
    vol_list = ['Penn Cove'] #['Salish Sea', 'Puget Sound', 'Hood Canal']
...
for which_vol in vol_list:
    ...
    # we use the "bulk_avg" output to get transport through the open boundaries
    bulk_dir = dir0 / ('bulk_avg' + date_str)
    
    # we use the "segments_avg" output to get the net tracer storage in the volume
    seg_ds_fn = dir0 / ('segments_avg' + date_str + '_' + sect_gctag + '_' + Ldir['riv'] + '.nc')
