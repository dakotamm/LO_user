...
def combineOverlaps(info_df_use):
    
    info_df_overlap
    
    for cid in info_df_use.index:
        
        info_df_use_temp = info_df_use[info_df_use['ij_list'] == info_df_use.loc[cid, 'ij_list']]
        
        new_cast = pd.DataFrame()
        
        new_cast = pd.concat([new_cast, info_df_use_temp.iloc[[0]]])
        
        cid_str = [str(x) for x in info_df_use_temp.index.tolist()]
        
        new_cast_cid = ''
        
        for c in cid_str:
        
            new_cast_cid = new_cast_cid + c + str(0)
            
        new_cast.index = pd.RangeIndex(int(new_cast_cid),int(new_cast_cid)+1)
        
        if ~int(new_cast_cid) in info_df_use.index:
        
            info_df_use = pd.concat([info_df_use, ])

                    
        # back shortly ****** 9/26/2023
...
def assignSurfaceToCasts(info_df_use, jjj, iii):
    ...
    if info_df_use.empty:
        ...
        ij_list = list(zip(info_df_use['ii_cast'], info_df_use['jj_cast']))
        
        for cid in info_df_use.index:
            
            ij_list_temp = tuple([info_df_use.loc[cid,'ii_cast'], info_df_use.loc[cid, 'jj_cast']])
            
            if ij_list.count(ij_list_temp) > 1:
                
                overlap_list.append(cid)
                
        ij_list_unique = list(set(ij_list))
        
        new_casts = []
        
        for ij in ij_list_unique:
            
            
          ### DM stopped week of 9/26/2023  
