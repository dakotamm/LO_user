...
# GOTTA LOAD IN DATABASE METADATA LIKE SOURCE AND TYPE****
...
for key in odf_dict.keys():
    ...
    month_counts_dict_temp = (pd.melt(odf_dict[key], id_vars=['cid', 'lon', 'lat', 'time', 'datetime', 'depth_bool', 'z', 'year', 'month', 'season', 'date_ordinal', 'segment'],
    ...
    month_counts_dict[key] = month_counts_dict_temp
...
for key in odf_dict.keys():
    ...
    seasonal_counts_dict_temp = (pd.melt(odf_dict[key], id_vars=['cid', 'lon', 'lat', 'time', 'datetime', 'depth_bool', 'z', 'year', 'month', 'season', 'date_ordinal', 'segment'],
                                    value_vars=var_list, var_name='var', value_name = 'val')
                         .dropna()
                         .set_index('datetime')
                         .groupby(['season', 'depth_bool', 'var']).agg({'cid' :lambda x: x.nunique()})
                         .reset_index()
                         )
