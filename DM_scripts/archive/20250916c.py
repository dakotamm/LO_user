...
all_stats_filt.loc[all_stats_filt['season'] == 'grow', 'season_label'] = 'Grow (Apr-Jul)'

all_stats_filt.loc[all_stats_filt['season'] == 'loDO', 'season_label'] = 'Lo-DO (Aug-Nov)'
...
for var in ['DO_mg_L', 'CT', 'SA']:
    ...
    fig, axd = plt.subplot_mosaic(mosaic, sharex=True, sharey=True, figsize=(8.5,2.5), layout='constrained', gridspec_kw=dict(wspace=0.1, hspace=0.1))
...
for var in ['DO_mg_L']:
    ...
    for season in ['Winter (Dec-Mar)', 'Grow (Apr-Jul)', 'Lo-DO (Aug-Nov)']:
        ...
        for site in long_site_list:
            ...
            for depth in ['Surface', 'Bottom']:
                ...
                ax.scatter(plot_df_['site_num'] + jitter[depth], plot_df_['slope_datetime_cent'], color=color_, marker=marker_, s=150, alpha= 0.5, label= 'Solubility-Based Trend') #, marker=markers[depth], edgecolors=edgecolors[site])
        ...
        if season == 'Winter (Dec-Mar)':
            ...
            selected_handles = [handles[0], handles[2], handles[3]]
            selected_labels = [labels[0], labels[2], labels[3]]
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + var +'_trends_wsol.png', dpi=500, transparent=True)
