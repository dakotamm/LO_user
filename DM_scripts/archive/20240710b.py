...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 12:27:00 2024
...
odf_dens_long = pd.melt(odf_dens, id_vars = ['site', 'summer_non_summer', 'year', 'date_ordinal','cid'], value_vars=['strat_sigma', 'strat_sigma_cons_T', 'strat_sigma_cons_S', 'DO_sol', 'DO_sol_cons_T', 'DO_sol_cons_S'], var_name='var', value_name='val')

# %%

odf_use = odf_depth_mean.copy()

odf_dens_use = odf_dens_long.copy()
...
for site in odf_dens_use['site'].unique():
    
    for season in ['summer']:
    
        mosaic = [['strat_sigma', 'DO_sol'], ['strat_sigma_cons_T', 'DO_sol_cons_T'], ['strat_sigma_cons_S', 'DO_sol_cons_S']]
            
        fig, ax = plt.subplot_mosaic(mosaic, layout='constrained', figsize = (10,8), sharex=True)
        ...
        for var in odf_dens_use['var'].unique():
           ...
           # ax[ax_name].text(0.99,0.99, 'slope = ' + str(np.round(slope_datetime,3)) + ' /yr +/- ' + str(np.round(sB1,6)), horizontalalignment='right', verticalalignment='top', transform=ax[ax_name].transAxes)
        
            ax[ax_name].text(0.99,0.99, 'slope = ' + str(np.round(slope_datetime*100,3)) + '/century +/- ' + str(np.round(slope_datetime_s*100,3)), horizontalalignment='right', verticalalignment='top', transform=ax[ax_name].transAxes, bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))
            ...
            if  'cons_S' in var:
                ...
                ax[ax_name].text(0.01,0.99, 'cons salinity (avg)', horizontalalignment='left', verticalalignment='top', transform=ax[ax_name].transAxes, bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))
                
            elif 'cons_T' in var:
                
                ax[ax_name].text(0.01,0.99, 'cons temperature (avg)', horizontalalignment='left', verticalalignment='top', transform=ax[ax_name].transAxes, bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))

            else:
                
                ax[ax_name].text(0.01,0.99, 'observed', horizontalalignment='left', verticalalignment='top', transform=ax[ax_name].transAxes, bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))

         
    
        plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_' + season + '_strat_sol_allcasts.png', bbox_inches='tight', dpi=500, transparent=False)
...
for site in odf_use['site'].unique():
    ...
    fig, ax = plt.subplot_mosaic(mosaic, layout='constrained', figsize = (5,8), sharex=True)
    ...
    for var in ['DO_mg_L', 'CT', 'SA']:
        ...
        for season in ['summer']:
            ...
            sns.scatterplot(data=plot_df, x='datetime', y ='val', hue = 'surf_deep', palette=colors, ax=ax[ax_name], alpha=0.7, legend = False, marker=marker)
            ...
            for depth in ['surf', 'deep']:
                ...
                y = plot_df[plot_df['surf_deep'] == depth]['val']
                ...
                sB1 = plot_df[plot_df['surf_deep'] == depth]['linreg_sB1'].unique()[0]
                ...
                ax[ax_name].text(0.99,y, 'slope = ' + str(np.round(slope_datetime*100,3)) + '/century +/- ' + str(np.round(slope_datetime_s*100,3)), horizontalalignment='right', verticalalignment='top', transform=ax[ax_name].transAxes, color=color, bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_surf_deep_decadal_nostrat_allcasts.png', bbox_inches='tight', dpi=500, transparent=False)
