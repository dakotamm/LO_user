...
for i in [0,1,2,3]:
    ...
    pcm = ax.pcolormesh(lat2d, z_rho_section, vel[i]*-1, vmin = -0.2, vmax = 0.2, cmap=cmc.broc_r)
        
    fig.colorbar(pcm, ax=ax, label="Velocity (m/s)")
    
    ax.set_title(Ldir['ds0']+ '-' + Ldir['ds1'])
        
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/pc0_section_vel_' + Ldir['ds0']+ '-' + Ldir['ds1'] + '.png', dpi=500, transparent=True, bbox_inches='tight')
