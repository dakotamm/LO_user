...
markers = {'surf': '^', 'deep': 'v'}
palette = {'point_jefferson': 'red', 'near_seattle_offshore': 'orange', 'carr_inlet_mid':'blue', 'saratoga_passage_mid':'indigo', 'lynch_cove_mid': 'purple'}
...
for var in var_list:
    ...
    plot_df = all_stats_filt[(all_stats_filt['var'] == var) & (all_stats_filt['season'] != 'allyear') & (all_stats_filt['site'] != 'near_seattle_offshore')]

    sns.scatterplot(data = plot_df, x = 'season', y= 'slope_datetime', hue ='site', style = 'surf_deep', ax=ax, legend=False)
    
plt.savefig('/Users/dakotamascarenas/Desktop/pltz/no_nso_sites_trends_comp.png', dpi=500)

# %%

odf_depths = odf_use.groupby(['site']).first().reset_index()
...
for var in var_list:
    ...
    plot_df = pd.merge(plot_df, odf_depths[['site', 'min_segment_h']], how='left', on='site')
    
    sns.scatterplot(data = plot_df, x = 'season', y= 'slope_datetime', hue ='min_segment_h', style = 'surf_deep', ax=ax, palette='plasma', legend=False)
    
plt.savefig('/Users/dakotamascarenas/Desktop/pltz/no_nso_sites_trends_comp_by_depth.png', dpi=500)
