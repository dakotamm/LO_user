...
for m in ms:
    ...
    fig.suptitle(m)
    plt.tight_layout()
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/'+ m + '_pc0_lowpass_CT_SA_DO_u_v.png',
...
for m in ['M2', 'M3', 'M4']:
    ...
    mesh = axs[c].pcolormesh(u['ocean_time'], z_rho.T, u.T,
                              shading='auto', cmap=cmocean.cm.balance,
                              vmin=vel_range[0], vmax=vel_range[1])
    fig.colorbar(mesh, ax=axs[c], label='m/s')
    axs[c].set_ylabel('Depth (m)')
    axs[c].set_title(m)
...
#fig.suptitle(m)
plt.tight_layout()
plt.savefig('/Users/dakotamascarenas/Desktop/pltz/M2_M3_M4_pc0_lowpass_u.png',
            dpi=500, transparent=True, bbox_inches='tight')
