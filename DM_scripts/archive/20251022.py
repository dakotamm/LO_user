...
df_m['NO3'] = df_m['NO3']*71.4 #following Aurora's conversions

df_m['NH4'] = df_m['NH4']*71.4
...
for site in df['name'].unique():
    ...
    for var in df['variable'].unique():
        ...
        plot_df = df[(df['name'] == site) & (df['variable'] == var)]
        
        ax.plot(plot_df['date'], plot_df['value'])
        
        plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_' + var +'.png',
                    dpi=500, transparent=False, bbox_inches='tight')
        
        
# %%

df_wide = df.pivot_table(index = ['date', 'name', 'year', 'month', 'day'], columns = ['variable'], values=['value'])
...
df_wide['din_kg_day'] = df_wide['NO3'] + df_wide['NH4'] * df_wide['flow'] * 14.01 * 60 * 60 * 24 / 1000 / 1000

# %%

for site in df_wide['name'].unique():
        
    fig, ax = plt.subplots()
    
    plot_df = df_wide[df_wide['name'] == site]
    
    ax.plot(plot_df['date'], plot_df['din_kg_day'])
    
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_din_kg_day.png',
                dpi=500, transparent=False, bbox_inches='tight')
    
# %%

df_wide_sum = df_wide.groupby(['date']).mean(numeric_only=True).reset_index()
...
plt.savefig('/Users/dakotamascarenas/Desktop/pltz/wwtp_din_kg_day.png',
            dpi=500, transparent=False, bbox_inches='tight')

ax.set_ylabel('Nutrient Load [kg/day]')
