...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  5 15:16:36 2024
...
monthly_skagit_df['val'] = monthly_skagit_df['mean_va']*0.02831683199881 #convert to m^3/s
...
ax[0].plot(monthly_skagit_df['datetime'], monthly_skagit_df['val'], color= 'gray', label = 'Monthly Measured Flow')
...
if p_value_all < 0.05:
    ...
    ax[0].plot([x_plot.min(), x_plot.max()], [all_result.intercept + all_result.slope*x.min(), all_result.intercept + all_result.slope*x.max()], alpha =0.7, linestyle = 'dashed', linewidth=2, label='All Year')
...
if p_value_loDO < 0.05:
    ...
    ax[0].plot([x_plot.min(), x_plot.max()], [loDO_result.intercept + loDO_result.slope*x.min(), loDO_result.intercept + loDO_result.slope*x.max()], alpha =0.7, linestyle = 'dashed', linewidth=2, label='Low-DO')
...
ax[0].set_ylabel('Monthly Mean Discharge [m^3/s]')

ax[1].set_ylabel('Decadal Mean Discharge [m^3/s]')
...
plt.savefig('/Users/dakotamascarenas/Desktop/pltz/fig4_test.png', bbox_inches='tight', dpi=500, transparent=False)