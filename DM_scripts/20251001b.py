...
mosaic = [['zeta'], ['salt'], ['temp'], ['oxygen']]

moor_dir = Ldir['LOo'] / 'extract'

for m in ['M1', 'M2', 'M3', 'M4', 'M5']:
    
    fig, axd = plt.subplot_mosaic(mosaic, figsize=(8,8), layout='constrained', sharex=True, gridspec_kw=dict(wspace=0.1))

    fig.suptitle(m + ' 2017 bottom')
    
    for g in ['cas7_t0_x4b', 'wb1_r0_xn11b']:
        
        fn = m + '_2017.01.01_2017.12.31.nc'
        
        moor_fn = moor_dir / g / 'moor' / 'pc0'/ fn
        ...
        for v in ['zeta','salt','temp','oxygen']:
            ...
            if 'cas7' in g:
                ...
                ax.text(0.05,0.95, v, transform=ax.transAxes, verticalalignment='top', fontweight = 'bold', color='k')
            ...
            if v == 'oxygen':
                ...
                ax.plot(df.index, df[v]*32/1000, color = color) 
            
            else:
                
                ax.plot(df.index, df[v], color = color)
        ...
        # df.plot(subplots=True, grid=True, fig=fig, title=m)
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + m+'.png', dpi=500,transparent=True, bbox_inches='tight')
