...
for summer in summers_list:
    ...
    for time_avg in time_avg_list:
        ...
        odf_use_dict[summer][time_avg] = odf_use
        
        odf_calc_use_dict[summer][time_avg] = odf_calc_use
                    ...
                    for var in var_list:
                        ...
                        for stat in ['linreg', 'mk_ts']:
                            ...
                            if stat == 'linreg':
                                ...
                                plot_df_concat = plot_df[['site', 'time_avg', 'summer_def','stat','var', 'p', 'slope_datetime_unc_cent', 'slope_datetime', 'slope_datetime_s', 'B1', 'B0']].head(1)
...
annual_skagit_df['year'] = annual_skagit_df['year_nu']


# %%

mosaic = [['surf_DO_mg_L', 'surf_CT', 'surf_SA', 'DO_sol'],['deep_DO_mg_L','deep_CT','deep_SA','strat_sigma']]

for summer in ['Aug-Nov']:
    
    for time_avg in ['annual']:
        
        odf_use = odf_use_dict[summer][time_avg]
        
        odf_calc_use = odf_calc_use_dict[summer][time_avg]
        
        odf_use = pd.merge(odf_use, annual_skagit_df[['year', 'mean_va']], how='left', on = ['year'])
        
        odf_use_temp = odf_use.copy()
        
        odf_use_temp['var'] = odf_use_temp['surf_deep'] + '_' + odf_use_temp['var']
        
        odf_calc_use = pd.merge(odf_calc_use, annual_skagit_df[['year', 'mean_va']], how='left', on = ['year'])
        
        odf_use_concat = pd.concat([odf_use_temp, odf_calc_use]) 
        
        for site in odf_use_concat['site'].unique():
            
            fig, ax = plt.subplot_mosaic(mosaic, layout='constrained', figsize=(16,9), sharex=True)
            
            for var in odf_use_concat['var'].unique():
                
                ax_name = var
            
                plot_df = odf_use_concat[(odf_use_concat['site'] == site) & (odf_use_concat['summer_non_summer'] == 'summer') & (odf_use_concat['var'] == var)]
        
                r_2 = (plot_df['mean_va'].corr(plot_df['val']))**2
        
                sns.scatterplot(data=plot_df, x='mean_va', y='val', hue ='year',ax=ax[ax_name], palette='plasma_r')
                
                ax[ax_name].text(0.98,0.98, 'r^2 = ' + str(np.round(r_2, 2)), horizontalalignment='right', verticalalignment='top', transform=ax[ax_name].transAxes, color='k', bbox=dict(facecolor='white', alpha=0.9, edgecolor='none'))
                
                ax[ax_name].legend(loc='lower right')
                
                ax[ax_name].set_ylabel(var)
                
                ax[ax_name].grid(color = 'lightgray', linestyle = '--', alpha=0.5)
                
                ax[ax_name].set_xlabel('Mean Discharge [cfs]')
                
                if 'DO' in var:
                    
                    ax[ax_name].axhspan(0,2,color='lightgray', alpha=0.2)
                    
                    ax[ax_name].set_ylim(bottom=0)
                    
                    
         
    
            plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + site + '_' + summer + '_' + time_avg + '_all_v_skagit.png', bbox_inches='tight', dpi=500, transparent=False)