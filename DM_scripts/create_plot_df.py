...
if info_fn.exists() & fn.exists():
    ...
    with open((file_dir + '/' + 'sub_avg_obs_T.pkl'), 'rb') as f:
        sub_avg_obs_T = pickle.load(f)
...
avg_df['T_avg_below_deg_C'] = []

avg_df['S_avg_below_g_kg'] = []
...
for seg_name in seg_list:
    ...
    for (mon_num, mon_str) in zip(month_num,month_str):
        ...
        if info_fn.exists() & fn.exists():    
            ...
            df_temp['T_avg_below_deg_C'] = [sub_avg_obs_T[seg_name][int(mon_num)]]
            
            df_temp['S_avg_below_g_kg'] = [sub_avg_obs_S[seg_name][int(mon_num)]]
