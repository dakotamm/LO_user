...
for i in [0,1,2,3]:
    ...
    lons = [-122.65741176, -122.65741176]
    
    from matplotlib.patches import Polygon
    
    ax.plot(lons, lats, color='0.5', lw=2)  # grey


    
    # Auto-size triangles relative to axes span (tweak these fractions if you want)
    xrel = ax.get_xlim()[1] - ax.get_xlim()[0]
    yrel = ax.get_ylim()[1] - ax.get_ylim()[0]
    arrow_len = 0.005 * xrel     # how far the tip sticks out to the left
    arrow_half_ht = 0.0025 * yrel # half the triangle height
    
    for x, y in zip(lons, lats):
        tri = Polygon(
            [
                (x - arrow_len, y),                 # TIP (left/west)
                (x, y + arrow_half_ht),             # base top (on section)
                (x, y - arrow_half_ht),             # base bottom (on section)
            ],
            closed=True,
            facecolor='0.5',
            edgecolor='0.5'
    ...
    fig.colorbar(cs, ax=ax, label = 'Bottom [DO] [mg/L]')
