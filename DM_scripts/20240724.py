...
for var in all_stats['var'].unique():
    ...
    for stat in ['linreg', 'mk_ts']:
        ...
        for time_avg in ['annual', 'none']:
            ...
            sns.scatterplot(data = plot_df[plot_df['p'] >= alpha], x= 'site_num', y = 'slope_datetime_cent', style = 'summer_def', style_order=['~85% hypoxic days','Aug-Sep','Aug-MidOct','Aug-Oct','Aug-MidNov','Aug-Nov'], color = 'lightgray', alpha =0.5, ax = ax[ax_name], legend=False)
            
            if ax_name == 'mk_ts_none':
                
                sns.scatterplot(data = plot_df[plot_df['p'] < alpha], x= 'site_num', y = 'slope_datetime_cent', style = 'summer_def', style_order=['~85% hypoxic days','Aug-Sep','Aug-MidOct','Aug-Oct','Aug-MidNov','Aug-Nov'], ax = ax[ax_name], alpha = 0.7, legend=True)

                ax[ax_name].legend(loc='upper left', bbox_to_anchor= (1,1))
            
            else:
                
                sns.scatterplot(data = plot_df[plot_df['p'] < alpha], x= 'site_num', y = 'slope_datetime_cent', style = 'summer_def', style_order=['~85% hypoxic days','Aug-Sep','Aug-MidOct','Aug-Oct','Aug-MidNov','Aug-Nov'], ax = ax[ax_name], alpha = 0.7, legend=False)
             
                        
            ax[ax_name].set_xticks([20,21,22,23,24],site_labels, rotation=90) #, ha='right')
