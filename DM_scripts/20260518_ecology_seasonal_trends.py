...
odf.loc[odf['month'].isin([1,2,3,12]), 'season'] = 'Winter (Dec-Mar)'
    
odf.loc[odf['month'].isin([4,5,6,7]), 'season'] = 'Grow (Apr-Jul)'
    
odf.loc[odf['month'].isin([8,9,10,11]), 'season'] = 'Lo-DO (Aug-Nov)'
...
# per-cast metadata (one row per cid) used to re-attach season / month to averaged frames
cast_meta = odf.groupby('cid').agg(season=('season', 'first'),
                                   month=('month', 'first'),
...
mins_dict['cast_bottom20p'] = mins_dict['cast_bottom20p'].drop(columns=['month','year'], errors='ignore').merge(cast_meta, on='cid', how='left')
...
mins_dict['cast_bottom5m'] = odf[odf['z'] < odf['h']+5].groupby(['cid','name','var']).mean(numeric_only=True).dropna().reset_index()
mins_dict['cast_bottom5m'] = mins_dict['cast_bottom5m'].drop(columns=['month','year'], errors='ignore').merge(cast_meta, on='cid', how='left')
mins_dict['cast_bottom5m'] = mins_dict['cast_bottom5m'].assign(datetime=(lambda x: x['date_ordinal'].apply(lambda x: pd.Timestamp.fromordinal(int(x)))))
...
def linRegDM(plot_df):
    ...
    # guard against empty or too-small input (need >=2 distinct x values)
    if len(x) < 2 or x.nunique() < 2:
        empty = plot_df.head(1).copy() if len(plot_df) else pd.DataFrame({'name': [np.nan], 'var': [np.nan]})
        for col in ['p', 'n', 'slope_datetime', 'slope_datetime_s_hi',
                    'slope_datetime_s_lo', 'B1', 'B0']:
            empty[col] = np.nan
        empty['n'] = len(x)
        return empty[['name','var','p','n','slope_datetime','slope_datetime_s_hi','slope_datetime_s_lo','B1','B0']]
...
min_type_list = ['cast_mins', 'cast_maxdepth', 'cast_bottom20p', 'cast_bottom5m']

season_list = ['allyear', 'winter', 'grow', 'loDO', 'Jul-Nov']
season_colors = {
    'allyear': '0.3',
    'winter':  'steelblue',
    'grow':    'seagreen',
...
for min_type in min_type_list:
    ...
    for site in df['name'].unique():
        ...
        for season in season_list:
            ...
            plot_df = df[(df['name'] == site)
                         & (df['month'].round().astype('Int64').isin(months))].copy()
...
out_dir = Ldir['LOo'] / 'DM_outs' / '20260518_ecology_seasonal_trends'
...
for site in sorted(all_sites):
    ...
    # shared y-axis limits across all min_types for this site
    all_vals = pd.concat([
    ...
    fig, axes = plt.subplots(len(min_type_list), len(season_list),
                             figsize=(4 * len(season_list), 3 * len(min_type_list)),
                             sharex=True, sharey=True, squeeze=False)
    fig.suptitle(site, fontsize=12, fontweight='bold')

    for i_row, min_type in enumerate(min_type_list):

        df_site = mins_dict[min_type][mins_dict[min_type]['name'] == site].copy().sort_values('datetime')

        for i_col, season in enumerate(season_list):
            ax = axes[i_row, i_col]
            ...
            sdf = df_site[df_site['month'].round().astype('Int64').isin(months)]

            ax.scatter(sdf['datetime'], sdf['val'], s=10, alpha=0.5, color=color)

            trend_row = min_trend_df[
                (min_trend_df['min_type'] == min_type)
                & (min_trend_df['name'] == site)
                & (min_trend_df['season'] == season)
            ]

            if not trend_row.empty and not sdf.empty:
                B0 = trend_row['B0'].values[0]
                B1 = trend_row['B1'].values[0]
                p_val = trend_row['p'].values[0]
                slope = trend_row['slope_datetime'].values[0]

                if not (pd.isna(B0) or pd.isna(B1)):
                    x_ord = sdf['date_ordinal'].values
                    x_line_ord = np.array([x_ord.min(), x_ord.max()])
                    x_line_dt = [pd.Timestamp.fromordinal(int(v)) for v in x_line_ord]
                    trend_vals = B0 + B1 * x_line_ord

                    sig = (not pd.isna(p_val)) and (p_val < 0.05)
                    ls = '-' if sig else '--'
                    lw = 2.0 if sig else 1.0
                    sig_label = '(p<0.05)' if sig else '(n.s.)'

                    ax.plot(x_line_dt, trend_vals,
                            color=color, linestyle=ls, linewidth=lw)
                    ax.set_title(f'{slope:.3f} mg/L/yr (p={p_val:.3f}) {sig_label}',
                                 fontsize=7)

            if i_row == 0:
                ax.text(0.5, 1.18, season, transform=ax.transAxes,
                        ha='center', va='bottom', fontsize=10, fontweight='bold')
            if i_col == 0:
                ax.set_ylabel(f'{min_type}\nDO (mg/L)', fontsize=8)
            if i_row == len(min_type_list) - 1:
                ax.set_xlabel('Date')
