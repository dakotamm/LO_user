...
big_df_use['time'] = pd.DatetimeIndex(big_df_use['TIMESTAMP'])
big_df_use['time'] = big_df_use['time'].dt.tz_localize('Etc/GMT+8')
big_df_use['time'] = big_df_use['time'].dt.tz_convert('UTC')
...
for year in year_list:
    ...
    if len(df) > 0:
        ...
        ds_df = df.copy()
        ds_df['time'] = ds_df['time'].dt.tz_localize(None)
        ds = (
            ds_df.set_index(["time", "z"])
