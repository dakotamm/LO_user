...
for year in year_list:
    ...
    if (year in range(2008,2026)) and (load_data == True):
        in_fn =  in_dir0 / 'SalishCruiseDataPackage_v2025_data_09072025.csv'
        df0 = pd.read_csv(in_fn, dtype={'TIME_UTC': str, 'DATE_UTC': str})
        # Fix hour=24 (midnight rollover convention) before parsing
        mask24 = df0['TIME_UTC'].str.startswith('24', na=False)
        df0.loc[mask24, 'TIME_UTC'] = '00' + df0.loc[mask24, 'TIME_UTC'].str[2:]
    ...
    df = df[df.name.notna()] # drop rows with no station name (e.g. STATION_NO=-999)
