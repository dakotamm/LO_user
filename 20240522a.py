...
# -*- coding: utf-8 -*-
"""
Created on Wed May 22 10:55:23 2024
...
odf_sal = odf_decadal_depth_mean_0.pivot(index = ['segment', 'year', 'summer_non_summer', 'date_ordinal', 'cid'], columns = ['surf_deep', 'var'], values ='val')
...
odf_sal = odf_sal[['segment', 'year','summer_non_summer', 'date_ordinal', 'cid', 'strat_psu']]
...
annual_counts_sal = (odf_decadal_depth_mean_0
                      .dropna()
                      #.set_index('datetime')
...
odf_sal_use = odf_sal.groupby(['segment', 'summer_non_summer', 'year']).agg({'strat_psu':['mean', 'std'], 'date_ordinal':['mean']})
...
odf_sal_use = pd.merge(odf_sal_use, annual_counts_sal, how='left', on=['segment','summer_non_summer','year'])
...
for basin in basin_list:
    ...
    mk_bool_decadal_sal[basin] = {}
    
    mk_p_decadal_sal[basin] = {}

        
    for season in ['summer', 'non_summer']:
        
                    
        mk_df = odf_sal_use[(odf_sal_use['segment'] == basin) & (odf_sal_use['summer_non_summer'] == season)]
        
        mk_df = mk_df.sort_values(by='datetime')
        
        reject_null, p_value, Z = dfun.mann_kendall(mk_df['strat_psu_mean'])
        
        mk_bool_decadal_sal[basin][season] = reject_null
        
        mk_p_decadal_sal[basin][season] = p_value
...
for big_basin in ['mb','wb','hc','ss']:
    ...
    for site in sites:
        ...
        for season in ['non_summer', 'summer']:
            ...
            plot_df = odf_sal_use[(odf_sal_use['segment'] == site) & (odf_sal_use['summer_non_summer'] == season)]
            ...
            ax[ax_name].set_ylabel(['Surf - Deep [psu]'])
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + big_basin + '_strat_psu_decadal.png', bbox_inches='tight', dpi=500)
...
mosaic = [['strat_psu_non_summer', 'strat_psu_summer']]
