...
fig, axd = plt.subplot_mosaic(mosaic, figsize=(12,8), layout='constrained', sharex=True, gridspec_kw=dict(wspace=0.1))
...
for var in ['DO_mg_L', 'CT', 'SA']:
    ...
    sns.scatterplot(data = plot_df, x= 'season_depth', y = 'val_ci95hi', hue ='surf_deep', palette = palette, style = 'summer_non_summer', markers={'summer':marker,'all':'.'}, ax = ax, s=10*multiplier,  legend=False)

    sns.scatterplot(data = plot_df, x= 'season_depth', y = 'val_ci95lo', hue ='surf_deep', palette = palette, style = 'summer_non_summer', markers={'summer':marker,'all':'.'}, ax = ax,  s=10*multiplier, legend=False)
    
    sns.scatterplot(data = plot_df, x= 'season_depth', y = 'val_mean', hue ='surf_deep', palette = palette, style = 'summer_non_summer', markers={'summer':marker,'all':'.'}, ax = ax,  s=50*multiplier, legend=False)
    ...
    for idx in plot_df.index:
        ...
        if plot_df.loc[idx,'surf_deep'] == 'surf':
            ...
            ax.plot([plot_df.loc[idx,'season_depth'], plot_df.loc[idx,'season_depth']],[plot_df.loc[idx,'val_ci95lo'], plot_df.loc[idx,'val_ci95hi']], color=palette['surf'], alpha =0.7, zorder = -4, linewidth=1)

        
        else:
                        
            ax.plot([plot_df.loc[idx,'season_depth'], plot_df.loc[idx,'season_depth']],[plot_df.loc[idx,'val_ci95lo'], plot_df.loc[idx,'val_ci95hi']], color=palette['deep'], alpha =0.7, zorder = -4, linewidth=1)
