...
poly_list = ['hc_wo_lc', 'lc']

odf_dict_temp = dfun.getPolyData(Ldir, poly_list)
...
odf_dict['lc'] = odf_dict_temp['lc']

odf_dict['hc_wo_lc'] = odf_dict_temp['hc_wo_lc']
            ...
            .assign(
               # datetime=(lambda x: pd.to_datetime(x['time'])),
                 depth_range=(lambda x: pd.cut(x['z'], 
                                               bins=[x['z'].min()-1, -355, -275, -205, -165, -135, -105, -80, -55, -37.5, -27.5, -17.5, -7.5, 0],
...
# SUPER MANUAL - come up with a better way maybe later
# - this ensures that lynch cove casts d

odf = odf[~odf['cid'].isin([6014, 6015, 6019, 6649, 6653, 6655, 6659, 6671, 19440, 19442, 19443])]
...
# make maps of decadal sampling locations
...
for basin in basin_list:
   ...
   # plot_df_mean = odf[(odf['segment'] == basin) & (odf['otype'] == 'bottle') & (odf['var'] == 'DO_mg_L')].groupby('decade').agg({'lat':'mean', 'lon':'mean'}).reset_index()
    ...
    sns.scatterplot(data=plot_df, x='lon', y='lat', hue='decade', palette='Set2') #, alpha=0.5)
...
# make maps of decadal sampling locations - for video!
...
for basin in basin_list:
    ...
    sns.scatterplot(data=plot_df, x='lon', y='lat', color = 'white', legend=False)
    ...
    for decade in plot_df['decade'].unique():
        ...
        plot_df_use = plot_df[plot_df['decade'] == decade]
    
   # plot_df_mean = odf[(odf['segment'] == basin) & (odf['otype'] == 'bottle') & (odf['var'] == 'DO_mg_L')].groupby('decade').agg({'lat':'mean', 'lon':'mean'}).reset_index()
    
        sns.scatterplot(data=plot_df_use, x='lon', y='lat', hue='decade', palette='crest', legend=False) #, alpha=0.5)
        
        ax.set_title(basin + ' ' + decade + ' bottle DO sampling locations')

        fig.tight_layout()
        
        plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + basin + '_bottle_DO_sampling_locations_by_decade_000' + str(c)+'.png', bbox_inches='tight', dpi=500)
...
for basin in ['hc_wo_lc']:
    ...
    for decade in plot_df['decade'].unique():
        ...
        fig, ax = plt.subplots(figsize=(40,40), nrows=1, ncols=4, squeeze=True)
        ...
        for season in ['winter', 'spring', 'summer', 'fall']:
            ...
            sns.scatterplot(data=plot_df, x='lon', y='lat', color = 'white', ax=ax[n], legend=False)
            
            ax[n].autoscale(enable=False)
        
            pfun.add_coast(ax[n])
        
            pfun.dar(ax[n])
            
            plot_df_use = plot_df[(plot_df['decade'] == decade) & (plot_df['season'] == season)]
    
   # plot_df_mean = odf[(odf['segment'] == basin) & (odf['otype'] == 'bottle') & (odf['var'] == 'DO_mg_L')].groupby('decade').agg({'lat':'mean', 'lon':'mean'}).reset_index()
    
            sns.scatterplot(data=plot_df_use, x='lon', y='lat', hue='decade', palette='crest', legend=False, ax=ax[n], s=100) #, alpha=0.5)
            
            ax[n].set_title(basin + ' ' + decade + ' ' + season)
            
            n+=1
                    
        #fig.set_title(basin + ' ' + decade + ' bottle DO sampling locations')

        fig.tight_layout()
    
        plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + basin + '_bottle_DO_sampling_locations_by_decade_by_season_individual_000' + str(c)+'.png', bbox_inches='tight', dpi=500)
...
for basin in basin_list:
        ...
        for season in ['winter', 'spring', 'summer', 'fall']:
            ...
            if not plot_df.empty:
                ...
                for decade in plot_df['decade'].unique():
                    ...
                    ax[c].fill_betweenx(plot_df[plot_df['decade'] == decade]['z_mean'], plot_df[plot_df['decade'] == decade]['val_ci95lo'], plot_df[plot_df['decade'] == decade]['val_ci95hi'], zorder=-4, alpha=0.4, color='lightgray')
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + basin + '_bottle_average_casts_decade_season_CI.png', dpi=500)
...
for basin in basin_list:
    ...
    plt.savefig('/Users/dakotamascarenas/Desktop/pltz/' + basin + '_bottle_average_casts_decade_season_CI_1940_2010.png', dpi=500)
